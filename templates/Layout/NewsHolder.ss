		...
		<% if FilteredUpdates %>
			...
			<% loop FilteredUpdates %>
				<article class="$EvenOdd">
					...
					<p class="metaInfo"><time <% if $Date %>datetime="$Date">$Date.nice<% if $StartTime %> $StartTime.Nice<% if $EndTime %> - $EndTime.Nice<% end_if %><% end_if %><% end_if %></time><% if Author %> by $Author<% end_if %></p>
					<p>
						<% if Abstract %>
							$Abstract
						<% else %>
